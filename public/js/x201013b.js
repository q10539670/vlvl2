...
var api = {
    ...
    * */
    ...
    reserve: function (data) {
        return new Promise(function (resolve, reject) {
            $.post(api.baseUrl + '/x201013b/reserve',
                /*接收参数-start*/
                {
                    name: data.name,                    //姓名:徐其阳
                    mobile: data.mobile,                //电话:18107120122
                    gender: data.gender,                //性别:[1:男,2:女]
                    id_num: data.id_num,                //身份证号码
                    on_foot: data.on_foot,              //是否有徒步经验[1:有,2:无]
                    date: data.date,                    //预约时间[17,24]
                    comment: data.comment               //身体状态自评
                },
                /*接收参数-end*/
                function (res) {
                    api.debug && console.log(res);
