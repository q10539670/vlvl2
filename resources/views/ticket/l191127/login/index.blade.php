...
{{--    <link rel="stylesheet" href="{{asset('vlvl/adminlte/bower_components/bootstrap/dist/css/bootstrap.min.css')}}">--}}
    ...
    <!-- iCheck -->
    <link rel="stylesheet" href="{{asset('vlvl/adminlte/plugins/iCheck/square/blue.css')}}">
...
<div class="login-box">
    <div class="login-logo">
        <a href="#"><b>武汉汤达人小票后台</b>登录<span style="font-size: 10px;margin-left: 12px;color:#595a5b;">第二期</span></a>
    </div>
    <!-- /.login-logo -->
    <div class="login-box-body">
        <p class="login-box-msg">请输入你的用户名和密码</p>

        <form action="{{route($loginRouteName)}}" method="post">
            @csrf
            @if(session('error'))
                <div class="form-group has-error">
                    <span class="help-block text-center">{{ session('error') }}</span>
                </div>
            @endif
...
<!-- iCheck -->
<script src="{{asset('vlvl/adminlte/plugins/iCheck/icheck.min.js')}}"></script>
<script>
    $(function () {
        $('input').iCheck({
