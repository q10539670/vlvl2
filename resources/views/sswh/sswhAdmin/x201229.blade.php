        ...
        <div class="box-header with-border">
            <div class="row">
                ...
                <div class="col-md-6 col-md-offset-2 col-sm-8 col-sm-offset-2">
                    <form class="form-inline" method="get" action="{{url('vlvl/x201229/index')}}">
                        ...
                        <div class="form-group">
                            <select name="status" class="form-control">
                                ...
                                <option {{($statusNow =='1')?'selected':''}} value="1">中奖未确认</option>
                                <option {{($statusNow =='2')?'selected':''}} value="2">未中奖</option>
                                <option {{($statusNow =='3')?'selected':''}} value="3">中奖已确认</option>
                            </select>
                        </div>
                        <div class="form-group">
    ...
    <script>
        function values(obj, id) {
            layer.prompt({title: '输入核销码', formType: 0}, function (text, index) {
                layer.close(index);
                $.ajax({
                    url: '{{url('vlvl/x201229/code')}}',
                    data: {id: id, code: text, _token: '{{csrf_token()}}'},
                    type: 'post',
