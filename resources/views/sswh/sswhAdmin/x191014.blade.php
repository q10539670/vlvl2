...
$verificationNow = request()->input('verification', '');
?>
@section('title', $title)
        ...
        <div class="box-header with-border">
            <div class="row">
                ...
                <div class="col-md-7 col-sm-8 col-sm-offset-2">
                    <form class="form-inline" method="get" action="{{url('vlvl/x191014/index')}}">
                        <div class="form-group">
                            <label class="sr-only" for="exampleInputEmail3"></label>
                            <input type="text"
                                   name="code"
                                   value="{{(request()->has('code')&&(request()->code!=''))?request()->code:''}}"
                                   class="form-control" size="16" placeholder="请输入中奖码查询">
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <select name="verification" class="form-control">
                                ...
                                <option {{($verificationNow =='0')?'selected':''}}  value="0">未核销</option>
                                <option {{($verificationNow =='1')?'selected':''}} value="1">已核销</option>
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <a href="{{url('vlvl/x191014/comm')}}" class="btn btn-info form-control">评论</a>
                        </div>
                    </form>
                </div>
            </div>
...
@section('js')
    <script>
        function values(obj, id) {
            layer.prompt({title: '输入中奖码', formType: 0}, function (text, index) {
                layer.close(index);
                $.ajax({
                    url: '{{url('vlvl/x191014/verification')}}',
                    ...
                    success: function (res) {
                        if (res == 1) {
                            ...
                            layer.msg('核销失败,请检查中奖码是否正确!', {icon: 5, time: 1000});
                        }
                    }
                })
            })
