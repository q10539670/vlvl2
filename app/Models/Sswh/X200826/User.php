...
namespace App\Models\Sswh\X200826;

use Illuminate\Database\Eloquent\Model;

class User extends Model
{
    protected $table = 'x200826_user';
   ...
   * #######################################################################################################################################################
   * */
    private $zhongJiangLv = 0.2;  //设置中奖率 如果大于 1 始终会转化为0~1之间的小数
    ...
    protected $prizeConf = [
        /*中奖数组*/
        'prize' => [
            0 => ['prize_id' => 1, 'prize_level_name' => '一等奖', 'prize_name' => '乐高积木', 'v' => 100, 'count' => 0, 'limit' => 0],
        ],
        /*不中奖   --未中奖*/
     ...
     * 随机抽奖  中奖几率始终不变
     * */
    public function fixRandomPrize($redisCountKey, $release)
    {
        ...
        $prizeCountKey = $redisCountKey . ':' . $release;
        $redis = app('redis');
        $redis->select(12);
...
//            dd($prizeCountKey);
        ...
        foreach ($this->prizeConf['prize'] as $k => $arr) {
            ...
            $this->prizeConf['prize'][$k]['limit'] = $this->prizeConfLimit[$release][$moneyStr]['limit'];
