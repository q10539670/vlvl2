...
class X191022Controller extends Controller
{
    ...
    protected $title = '全网通（10月）游戏红包抽奖';
...
//        dd($user);
        return view('sswh.qwt.x191022.index', [
            'paginator' => $user,
            'countData' => $countData,
            'title' => $this->title,
            'areaUrl' => 'https://wx.sanshanwenhua.com/vlvl/x191022/area'
        ]);
    }
...
//        dd(json_decode($hubeiBarData,true)['name']);
        return view('sswh.qwt.x191022.area', [
            ...
            'countData' => $countData,
            'title' => $this->title,
            'indexUrl' => 'https://wx.sanshanwenhua.com/vlvl/x191022/index'
        ]);
    }
...
//            'test',
        ...
        $redisShareData = Helper::getRedisShareData('flip_clear2');
        $res = [];
        ...
        foreach ($dateArr as $dateStr) {
            $redisPrizeCountKey = 'qwt:'.$this->itemName.':prizeCount';
    ...
    {
        $a = China::where("pid", "420200")->get()->pluck("id");

        $data = User::whereIn("virtual_address_code", $a)/*->orderBy("address_code", "desc")*/->get()
            ->map(function ($item, $key) {
                return [
                    '昵称' => $item['nickname'],
                    '用户选择地址' => $item['virtual_address_str'],
                    '微信定位地址' => $item['address_str'],
                    '进入时间' => $item['created_at']
                ];
            });

        Excel::create("用户选择黄石6月-用户详情", function ($excel) use ($data) {
...
//        $res['five'] = User::whereIn('virtual_address_code', $areaIdArr)->where('status', 1)->where('money', 5)->count();
    ...
    protected function getAreaCountData()
    {
        ...
        foreach ($shengs as $k => $v) {
            ...
            $shengsCount[] = [
                'name' => $v['name'],
                'currentNum' => User::/*where('status',1)->*/ whereIn('virtual_address_code', $areaIdArr)->count(), //统计用户选择的
//                'currentNum' => User::/*where('status',1)->*/whereIn('address_code', $areaIdArr)->count(),  //统计经纬度定位的
        ...
        foreach ($hbAreaIdArr as $k => $v) {
            ...
            $hbAreaIdArr[$k]['currentNum'] = User::whereIn('virtual_address_code', $areaIdArr)/*->where('status',1)*/ ->count(); //统计用户选择的
