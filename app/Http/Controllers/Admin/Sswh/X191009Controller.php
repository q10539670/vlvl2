...
use App\Exports\X191009Export;
use App\Http\Controllers\Common\BaseV1Controller as Controller;

use App\Models\Sswh\X191009\User;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class X191009Controller extends Controller
{
    public function index(Request $request)
    {
        ...
        $exportUrl = asset('/vlvl/x191009/export');
        $redis = app('redis');
        $redis->select(0);
        $redisShareData = $redis->hGetAll('wx:view:k_20191008');
        return view('sswh.sswhAdmin.x191009', ['title' => '长沙·洋湖天街', 'paginator' => $paginator, 'exportUrl' => $exportUrl, 'redisShareData' => $redisShareData]);
    }

    public function export()
    {
        return Excel::download(new X191009Export, '长沙·洋湖天街中奖名单.xlsx');
    }
