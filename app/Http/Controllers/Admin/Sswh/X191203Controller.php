...
use App\Exports\X191203Export;
use App\Http\Controllers\Common\BaseV1Controller as Controller;

use App\Models\Jyyc\X191203\User;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class X191203Controller extends Controller
{
    public function index(Request $request)
    {
        ...
        $exportUrl = asset('/vlvl/x191203/export');
        $redis = app('redis');
        $redis->select(0);
        $redisShareData = $redis->hGetAll('wx:view:lh20191126');
        return view('sswh.sswhAdmin.x191203', ['title' => '宜昌中心·天宸府现金抽奖', 'paginator' => $paginator, 'exportUrl' => $exportUrl, 'redisShareData' => $redisShareData]);
    }

    public function export()
    {
        return Excel::download(new X191203Export, '宜昌中心·天宸府参与名单.xlsx');
    }
}
