...
use App\Exports\X191122Export;
use App\Http\Controllers\Common\BaseV1Controller as Controller;
use App\Models\Sswh\X191122\User;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class X191122Controller extends Controller
{
    /**
     * 首页
    ...
    public function index(Request $request)
    {
        ...
        $exportUrl = asset('/vlvl/x191122/export');
        $redis = app('redis');
        $redis->select(0);
        $redisShareData = $redis->hGetAll('wx:view:l_20191122');
        return view('sswh.sswhAdmin.x191122', ['title' => '百事感恩节', 'paginator' => $paginator, 'exportUrl' => $exportUrl, 'redisShareData' => $redisShareData]);
    }

    public function export()
    {
        return Excel::download(new X191122Export(), '百事感恩节抽奖名单.xlsx');
    }
}
