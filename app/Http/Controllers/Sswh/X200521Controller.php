...
class X200521Controller extends Common
{

    //中国中铁·金桥璟园答题领红包
    protected $itemName = 'x200521';

    //红包开关
    const OPEN_SEND_REDPACK = false;
    const END_TIME = '2020-05-22 20:04:59';  //活动截止时间
    const START_TIME = '2020-05-22 20:00:00';  //活动开始时间
    ...
    public function user(Request $request)
    {
        ...
        $redis->select(0);
        $redis->hIncrBy('wx:view:jq20200521','view',1);
        $redis->hmset('wx:view:jq20200521','lastViewTime',time());
        return $this->returnJson(1, "查询成功", [
            'user' => $user,
            'prize_num' => $prizeNum,
            'is_active_time' => ($this->isTimeout(self::END_TIME,self::START_TIME)) ? 0 : 1,
    ...
    public function randomPrize(Request $request)
    {
        ...
        if ($timeStr > '2020-05-22 20:00:00') {
            $timeStr = 'gold';
        } else {
            $timeStr = 'test';
        }
