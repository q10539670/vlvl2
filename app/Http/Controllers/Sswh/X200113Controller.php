...
use App\Models\Sswh\X200113\VoteLog;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;

class X200113Controller extends Common
{
    //湘中投票
    protected $itemName = 'x200113';

    const START_TIME = '2020-01-17 09:00:00';
    const SUBMIT_TIME = '2020-02-10 20:00:00';
    const VOTE_START = '2020-02-11 09:00:00';
    const END_TIME = '2020-02-20 20:00:00';
    ...
    {
        $perPage = $request->has('per_page') ? $request->per_page : 4; //每页显示数量
        ...
        if ($request->order == 'news') {
            $query->orderBy('img_at', 'desc');
        } else {
            $query->orderBy('polls', 'desc')->orderBy('img_at', 'asc');
        }
    ...
    public function post(Request $request)
    {
        ...
        if ($user->status == 1) {
            return response()->json(['error' => '您已经上传不合格照片3次，没有上传机会了哟~'], 422);
        }
    ...
    public function vote(Request $request)
    {
        if (time() < strtotime(self::VOTE_START)) {
            return response()->json(['error' => '投票时间未开始']);
        }
