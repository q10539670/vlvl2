...
//opcache_reset();
class X191225Controller extends Common
{

        //钢材商会
    protected $itemName = 'x191225';

    const OPEN_SEND_REDPACK = false;  //红包开关
    const START_TIME = true; //开始时间
    const END_TIME = '2019-12-29 16:20:59';  //结束时间
    ...
    public function post(Request $request)
    {
        ...
        if (time() > strtotime(self::END_TIME)) {
            return response()->json(['error' => '红包派发完毕'], 422);
        }
    ...
    public function randomPrize(Request $request)
    {
        ...
        if (time() > strtotime(self::END_TIME)) {
            return $this->returnJson(1, ['error' => '红包派发完毕']);
        }
        if ($user->status == 1) {
...
//    {
//        $user = new User();
//        $resultRedpack = $user->sendRedpack(122, 'o9kSI0eSxqwEBsZ92UwUgTnq27DE', 63, self::OPEN_SEND_REDPACK);
//        return Helper::json(1,'发送成功',['resultPrize'=>$resultRedpack]);
//    }
