...
class X191225bController extends Common
{
        //台州商会
    protected $itemName = 'x191225b';

    const OPEN_SEND_REDPACK = false;  //红包开关
    const START_TIME = '2019-12-24 09:00:00'; //开始时间
    ...
    public function randomPrize(Request $request)
    {
        ...
        if ($resultPrize['resultPrize']['money'] != 0) {
            if ($resultPrize['resultPrize']['money'] == 666) {
                $user->redpack_return_msg = '人工发放';
                $user->status = 1;
                $user->redpack_describle = '人工发放';
                $user->prize_num--;
                $user->money = 666*100;
                $user->prized_at = now()->toDateTimeString();
                $user->save();
                return Helper::json(1, '666元大红包',['user' => $user,'resultPrize'=>$resultPrize['resultPrize'],'money'=>666]);
            }
...
//            }
        ...
        $redis->hmset('wx:' . $this->itemName . ':prizeCount', [ '1' => 0, '2' => 0, '3' => 0, '20' => 0, '21' => 0, '22' => 0]);
        echo '应用初始化成功';
        exit();
    }
}
